...
const Ground = () => {
  ...
  const fetchGrounds = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${BACKEND_URL}/api/infrastructure/grounds`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${BACKEND_URL}/api/infrastructure/grounds/${editingId}`, formData);
      } else {
        await axios.post(`${BACKEND_URL}/api/infrastructure/grounds`, formData);
      }
      setFormOpen(false);
      fetchGrounds();
    } catch (error) {
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`${BACKEND_URL}/api/infrastructure/grounds/${itemToDelete._id}`);
  ...
  const tableRows = grounds.map((item, index) => [
    index + 1,
    item.name,
    {
      value: item.status,
  ...
  return (
    <Box sx={{ p: 3 }}>
      <PageHeader
        ...
        subtitle="Create and manage grounds for outdoor events"
        action={
          <Button
            variant="contained"
            startIcon={<AddIcon />}
