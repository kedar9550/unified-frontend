...
const Building = () => {
  ...
  const fetchBuildings = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${BACKEND_URL}/api/infrastructure/buildings`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`${BACKEND_URL}/api/infrastructure/buildings/${editingId}`, formData);
      } else {
        await axios.post(`${BACKEND_URL}/api/infrastructure/buildings`, formData);
      }
      setFormOpen(false);
      fetchBuildings();
    } catch (error) {
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`${BACKEND_URL}/api/infrastructure/buildings/${itemToDelete._id}`);
  ...
  const tableRows = buildings.map((item, index) => [
    index + 1,
    item.name,
    {
      value: item.status,
  ...
  return (
    <Box sx={{ p: 3 }}>
      <PageHeader
        ...
        subtitle="Create and manage buildings"
        action={
          <Button
            variant="contained"
            startIcon={<AddIcon />}
