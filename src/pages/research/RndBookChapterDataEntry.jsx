...
export default function RndBookChapterDataEntry() {
  ...
  const [form, setForm] = useState({
    doi: "", chapterTitle: "", bookTitle: "", publisher: "", isbn: "",
    month: "", year: "", indexing: "", scope: "",
  ...
  const handleSubmit = async () => {
    ...
    if (!form.chapterTitle || !form.bookTitle || !form.publisher || !form.isbn || !form.month || !form.year) {
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        subtitle="Add book chapter publication records directly on behalf of faculty members"
      />
      ...
      <FormCard title="2. Book Chapter Details" icon={<MenuBook sx={{ color: "var(--color-primary)" }} />}>
        <Grid container spacing={2.5}>
          ...
          <Grid item xs={12} sm={6} md={4}>
            ...
            <TextField fullWidth size="small" value={form.doi} onChange={set("doi")} />
          </Grid>
          <Grid item xs={12} sm={6} md={4}>
            <FieldLabel required>Chapter Title</FieldLabel>
            <TextField fullWidth size="small" value={form.chapterTitle} onChange={set("chapterTitle")} />
          </Grid>
          <Grid item xs={12} sm={6} md={4}>
            <FieldLabel required>Book Title</FieldLabel>
            <TextField fullWidth size="small" value={form.bookTitle} onChange={set("bookTitle")} />
          </Grid>
          <Grid item xs={12} sm={6} md={4}>
          ...
          <Grid item xs={12} sm={6} md={4}>
            <FieldLabel>Indexing</FieldLabel>
      ...
      <FormCard title="3. Attachments" icon={<AttachFile sx={{ color: "var(--color-primary)" }} />}>
        <Grid container spacing={2.5}>
          <Grid item xs={12} sm={6}>
            <FieldLabel required>Published Chapter Document</FieldLabel>
            <FileField onChange={(e) => setFiles(p => ({ ...p, publishedChapter: e.target.files[0] }))} />
          </Grid>
          <Grid item xs={12} sm={6}>
            <FieldLabel>Indexing Proof (Optional)</FieldLabel>
            <FileField onChange={(e) => setFiles(p => ({ ...p, indexProof: e.target.files[0] }))} />
          </Grid>
        </Grid>
      </FormCard>
