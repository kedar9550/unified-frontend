...
const NovelProductApprovalDetail = ({ id, onBack, role }) => {
    ...
    const handleAction = async (action) => {
        ...
        try {
            const endpoint = isResearchAdmin ? `/api/research/novel-product/rnd-action/${id}` : `/api/research/novel-product/hod-action/${id}`;
    ...
    const adityaCoDevNames = (data.coDevelopers || [])
        .filter(c => c.affiliationType === "Aditya University" || c.employeeId || c.affiliation === "Aditya University")
        .map(c => c.name)
        .filter(Boolean);
    const allClaimants = [mainFacultyName, ...adityaCoDevNames].filter((v, i, a) => v && a.indexOf(v) === i);
        ...
        <Box sx={{ width: "100%", pb: 5 }}>
            ...
            <Card sx={cardStyle}>
                <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, justifyContent: "space-between", alignItems: { xs: "center", sm: "flex-start" }, gap: 2, mb: 4 }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                        ...
                        <Box>
                            <Typography variant="h5" sx={{ fontWeight: 800, color: "var(--text-primary)" }}>{data.productName}</Typography>
                            <Typography variant="body2" sx={{ color: "var(--text-secondary)", fontWeight: 600 }}>Category: {data.category}</Typography>
                        </Box>
                    </Box>
                    <Box sx={{ textAlign: { xs: "center", sm: "right" } }}>
                        {(data.entryType === 'Admin' || data.isDirectEntry === 'true' || data.isDirectEntry === true) && (
                            <Chip label="R&D Direct Entry" sx={{ bgcolor: "rgba(124, 58, 237, 0.1)", color: "#7c3aed", border: "1px solid rgba(124, 58, 237, 0.3)", fontWeight: 800, borderRadius: "8px", fontSize: "0.65rem", mr: 1 }} />
                        )}
                        <Chip label="Novel Product / Tech" sx={{ bgcolor: "rgba(139, 92, 246, 0.1)", color: "#8B5CF6", fontWeight: 800, borderRadius: "8px", textTransform: "uppercase", fontSize: "0.65rem" }} />
                ...
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2, width: "100%" }}>
                    ...
                    <Box sx={{ flex: { xs: "1 1 100%", sm: "1 1 23%" } }}>
                        <LabelValue label="Reference ID" value={`TECH-${new Date(data.createdAt).getFullYear()}-${data._id.substring(data._id.length - 6).toUpperCase()}`} />
                    </Box>
                    <Box sx={{ flex: { xs: "1 1 100%", sm: "1 1 23%" } }}>
            ...
            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 3, mb: 3 }}>
                ...
                <Card sx={{ ...cardStyle, flex: { xs: "1 1 100%", lg: "1 1 48%" }, mb: 0 }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 3 }}><PersonIcon sx={{ color: "var(--color-primary)" }} /><Typography variant="h6" sx={{ fontWeight: 800, color: "var(--text-primary)" }}>Faculty Information</Typography></Box>
                ...
                {/* Product Details */}
                <Card sx={{ ...cardStyle, flex: { xs: "1 1 100%", lg: "1 1 48%" }, mb: 0 }}>
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mb: 3 }}><ArticleIcon sx={{ color: "var(--color-primary)" }} /><Typography variant="h6" sx={{ fontWeight: 800, color: "var(--text-primary)" }}>Technical Details</Typography></Box>
                    <Box sx={{ display: "flex", flexDirection: "column" }}>
                        <LabelValue label="Product/Tech Name" value={data.productName} horizontal />
                        <LabelValue 
                            label="Category" 
                            horizontal 
                            chip={
            ...
            <Card sx={cardStyle}>
                ...
                <Grid container spacing={3}>
                    {renderFilePreview("Documentation / Technical Report / Implementation Proof", data.document, 1)}
                </Grid>
            </Card>
                                    ...
                                    {decisionMode === 'Approve' && isResearchAdmin && (
                                        <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
                                            <Box sx={{ flex: "1 1 200px" }}>
                                                <Typography variant="subtitle2" sx={{ fontWeight: 900, mb: 1, color: "var(--color-primary)", fontSize: "0.75rem" }}>NOVEL PRODUCT ELIGIBILITY FOR APPRAISAL *</Typography>
                                                <Select 
...
export default NovelProductApprovalDetail;
