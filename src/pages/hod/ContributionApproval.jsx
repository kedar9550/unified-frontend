...
const LabelValueDetails = ({ label, value, chip, horizontal = false, display = null }) => (
  <Box sx={{
...
const ContributionApproval = () => {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const res = await API.get('/api/value-addition/contribution/pending-hod', {
    ...
    return (
      <Dialog open={!!selected} onClose={() => setSelected(null)} maxWidth="md" fullWidth sx={{ '& .MuiPaper-root': { borderRadius: '16px', background: 'var(--bg-panel)' } }}>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography variant="h6" sx={{ fontWeight: 800, color: 'var(--color-primary)' }}>Contribution Details</Typography>
