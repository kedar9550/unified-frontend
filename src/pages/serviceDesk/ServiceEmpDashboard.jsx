...
const StatCard = ({ title, value, iconColor, bg, sub, icon: SIcon }) => (
  <Paper sx={{
    p: 3, borderRadius: '16px', flex: '1 1 180px', minWidth: 0,
    background: 'var(--card-bg)', border: '1px solid var(--border-color)',
    boxShadow: '0 2px 12px rgba(0,0,0,0.05)', position: 'relative', overflow: 'hidden',
    transition: 'transform .2s', '&:hover': { transform: 'translateY(-3px)', boxShadow: '0 8px 24px rgba(0,0,0,0.1)' }
  }}>
    <Box sx={{ position: 'absolute', top: -20, right: -20, width: 90, height: 90, borderRadius: '50%', background: bg, opacity: 0.4 }} />
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 3 }}>
        <StatCard title="Total Assigned"   value={tickets.length}          icon={AssignmentInd} iconColor="#3b82f6" bg="rgba(59,130,246,0.12)"  />
        <StatCard title="Active Tasks"     value={activeTickets.length}    icon={HourglassTop}  iconColor="#f59e0b" bg="rgba(245,158,11,0.12)"  sub="Need action" />
        <StatCard title="In Progress"      value={inProgress.length}       icon={TrendingUp}    iconColor="#8b5cf6" bg="rgba(139,92,246,0.12)"  />
        <StatCard title="Resolved by Me"   value={resolvedTickets.length}  icon={CheckCircle}   iconColor="#10b981" bg="rgba(16,185,129,0.12)"  />
        <StatCard title="Urgent Tasks"     value={urgent.length}           icon={PriorityHigh}  iconColor="#ef4444" bg="rgba(239,68,68,0.12)"   sub="Critical + High" />
      </Box>
