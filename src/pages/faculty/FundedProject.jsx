        ...
        <DialogContent sx={{ p: 3, mt: 1 }}>
          ...
          <Paper elevation={0} sx={{ borderRadius: "16px", border: "1px solid var(--border-color)", background: "var(--bg-paper)", overflow: "hidden", mb: 3 }}>
            ...
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              {[
                ...
                { label: "Funding Agency", value: data.fundingAgency || "-" },
                { label: "Internal Funding (Aditya University)?", value: data.fundingAgencyAditya || "No" },
                { label: "Scheme", value: data.scheme || "-" },
                { label: "Duration (Years)", value: data.duration || "-" },
                { label: "Investigator Role", value: data.investigatorType || "N/A" },
                { label: "Sanctioned Amount", value: data.sanctionedAmount ? `₹${data.sanctionedAmount}` : "-" },
                { label: "Date of Sanction", value: formatDate(data.sanctionDate) },
                { label: "Recurring Amount", value: data.recurring ? `₹${data.recurring}` : "-" },
                { label: "Non-Recurring Amount", value: data.nonRecurring ? `₹${data.nonRecurring}` : "-" },
          ...
          <Paper elevation={0} sx={{ p: 3, mb: 3, borderRadius: "16px", border: "1px solid var(--border-color)", background: "var(--bg-paper)" }}>
            ...
            <Stack direction="row" spacing={3} sx={{ flexWrap: "wrap" }} useFlexGap>
              {renderDetailFile("Sanction Order Document", data.sanctionOrder)}
            </Stack>
          </Paper>
