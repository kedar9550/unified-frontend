        ...
        <Box sx={{ width: "100%", px: { xs: 1.5, sm: 2, md: 3 }, pb: 5 }}>
            ...
            <Card sx={cardStyle}>
                ...
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12} sm={3}><LabelValue label="Reference ID" value={`BCP-${new Date(data.createdAt).getFullYear()}-${data._id.substring(data._id.length - 6).toUpperCase()}`} /></Grid>
                    <Grid item xs={12} sm={3}><LabelValue label="Submission Date" value={new Date(data.createdAt).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })} /></Grid>
                    <Grid item xs={12} sm={3}>
                        <Box sx={{ p: 2, borderRadius: "14px", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                ...
                <Card sx={{ ...cardStyle, flex: { xs: "1 1 100%", lg: "1 1 48%" }, mb: 0 }}>
                    ...
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 3, alignItems: "center" }}>
                        <Box sx={{ width: 100, height: 100, borderRadius: "50%", background: "var(--bg-panel)", border: "2px solid var(--border-color)", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "var(--shadow-premium)" }}>
                            {facultyId?.profileImage ? <img src={`${import.meta.env.VITE_BACKEND_URL || "http://localhost:5000"}${facultyId.profileImage}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <Typography sx={{ fontSize: 36, fontWeight: 800, color: "var(--text-secondary)" }}>{facultyId?.name?.charAt(0)}</Typography>}
                        </Box>
                        <Box sx={{ width: "100%" }}>
